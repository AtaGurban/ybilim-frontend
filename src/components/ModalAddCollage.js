...
import { createCollage } from "../http/educaionApi";

const ModalAddCollage = ({ show, onHide, cityId }) => {
  ...
  const addCollage = async () => {
    ...
    try {
      await createCollage(formData, options).then((data) => {
        onHide();
...
export default ModalAddCollage;
